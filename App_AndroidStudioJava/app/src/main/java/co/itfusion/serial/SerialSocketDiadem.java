...
public class SerialSocketDiadem extends BluetoothGattCallback {
    ...
    private static final UUID BLUETOOTH_LE_BTM_DESCRIPTOR = UUID.fromString("00002902-0000-1000-8000-00805f9b34fb");
    ...
    private static final UUID BLUETOOTH_BTM_SERVICE = UUID.fromString("0000b350-0000-1000-8000-00805f9b34fb");
    private static final UUID BLUETOOTH_BTM_RX = UUID.fromString("0000b351-0000-1000-8000-00805f9b34fb"); //Used to Receive data FROM the module
    private static final UUID BLUETOOTH_BTM_TX = UUID.fromString("0000b352-0000-1000-8000-00805f9b34fb"); //Used for write data to the module
    ...
    private static final String TAG = "SerialSocketDIMA";
    ...
    public SerialSocketDiadem(Context context, BluetoothDevice device) {
    ...
    public void disconnect() {
        ...
        if (gatt != null) {
            ...
            gatt = null;
            connected = false;
            SerialUtils.statusBleService(context, SerialUtils.ServiceStatus.Disconnected);
        }
        try {
    ...
    public void connect(ISerialListener listener) throws IOException {
        if (connected || gatt != null)
            throw new IOException("already connected");
        canceled = false;
        this.listener = listener;
        context.registerReceiver(disconnectBroadcastReceiver, new IntentFilter(Permissions.INTENT_ACTION_DISCONNECT), Context.RECEIVER_NOT_EXPORTED);
        ...
        if (Build.VERSION.SDK_INT < 23) {
            ...
            Log.d(Constants.TAG, "gatt connected: " + gatt);
        }
        if (gatt == null)
...
//                onSerialConnectError(new IOException(context.getString(R.string.pairing_request)));
    ...
    private void connectCharacteristics1(BluetoothGatt gatt) {
        ...
        Log.d(TAG, "gatt detected: " + gatt.getServices());
        Log.d(TAG, "device: " + gatt.getDevice().getName());
        ...
        for (BluetoothGattService gattService : gatt.getServices()) {
            ...
            if(gattService.getUuid().equals(BLUETOOTH_BTM_SERVICE))
                delegate = new BtmDelegate();
//                delegate = new BtmDelegate2();

            if (delegate != null) {
                Log.d(Constants.TAG, "delegate is not null");
        ...
        if (sync){
            Log.d(Constants.TAG, "sync is true trying to connectCharacteristics2");
            connectCharacteristics2(gatt);
        } else Log.d(Constants.TAG, "sync is false?");
    }

    @SuppressLint({"MissingPermission", "ObsoleteSdkInt"})
    private void connectCharacteristics2(BluetoothGatt gatt) {
        Log.d(TAG, "request max MTU");
        boolean mtuRequest = gatt.requestMtu(MAX_MTU);
        if(mtuRequest) {
            Log.d(Constants.TAG, "mtu request successful, now what?");
        } else
    ...
    public void onMtuChanged(BluetoothGatt gatt, int mtu, int status) {
        Log.d(Constants.TAG, "onMTUChanged: callback");
    ...
    private void connectCharacteristics3(BluetoothGatt gatt) {
        Log.d(Constants.TAG, "attempting to connectCharacteristics3");
        Log.d(Constants.TAG, "checking write property");
        ...
        if ((writeProperties & (BluetoothGattCharacteristic.PROPERTY_WRITE +     // Microbit,HM10-clone have WRITE
            ...
            return;
        } else Log.d(Constants.TAG, "write characteristic is writable");

        Log.d(Constants.TAG, "checking read property");
        ...
        if (!gatt.setCharacteristicNotification(readCharacteristic, true)) {
            ...
            return;
        } else Log.d(Constants.TAG, "set characteristic notification is true");

        BluetoothGattDescriptor descriptor = readCharacteristic.getDescriptor(BLUETOOTH_LE_CCCD);
        if (descriptor == null) {
            onSerialConnectError(new IOException("no CCCD descriptor for read characteristic"));
            return;
        } else Log.d(Constants.TAG, "read descriptor is not null");
...
//        if (!gatt.writeDescriptor(descriptor)) {
        ...
        } else Log.d(Constants.TAG , "descriptor successfully attached");
    ...
    public void onDescriptorRead(BluetoothGatt gatt, BluetoothGattDescriptor descriptor, int status) {
        Log.d(Constants.TAG, String.format("onDescriptorRead: %s, %s, %s", gatt.getDevice().getName(), descriptor.getCharacteristic().getUuid(), status));
        if (canceled)
            return;
    ...
    public void onDescriptorWrite(BluetoothGatt gatt, BluetoothGattDescriptor descriptor, int status) {
        Log.d(Constants.TAG, String.format("onDescriptorWrite: %s, %s, %s", gatt.getDevice().getName(), descriptor.getCharacteristic().getUuid(), status));
...
//            Log.d(TAG, "read, len=" + data.length);
    ...
    private void onSerialConnect() {
        ...
        Log.d(Constants.TAG, "ON SERIAL CONNECT - SOCKET");
        SerialUtils.statusBleService(context, SerialUtils.ServiceStatus.Connected, device);
    }
    ...
    private void onSerialConnectError(Exception e) {
        ...
        SerialUtils.statusBleService(context, SerialUtils.ServiceStatus.Error);
        Log.d(Constants.TAG, "ON SERIAL CONNECT ERROR - SOCKET");
    ...
    private void onSerialIoError(Exception e) {
        ...
        Log.d(Constants.TAG, "ON SERIAL IO ERROR - SOCKET");
        SerialUtils.statusBleService(context, SerialUtils.ServiceStatus.ErrorIO);
    ...
    private class BtmDelegate extends DeviceDelegate {
        @SuppressLint("MissingPermission")
        @Override
        boolean connectCharacteristics(BluetoothGattService gattService) {
            Log.d(Constants.TAG, "BTM Delegate");
            readCharacteristic = gattService.getCharacteristic(BLUETOOTH_BTM_RX);
            writeCharacteristic = gattService.getCharacteristic(BLUETOOTH_BTM_TX);
            ...
            Log.d(Constants.TAG, "returning from  BTM characteristic");
            return true;
        }
    }
